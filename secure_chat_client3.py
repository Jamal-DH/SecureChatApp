...
• USB 2-factor picker (handles multiple sticks)
...
class ChatClient:
    ...
    def __init__(self, master: ctk.CTk, host: str,
        ...
        self.running = False
        self.recipient = "Everyone"         # GUI state
    ...
    def start(self) -> None:
        ...
        self.tls_ctx = configure_tls_context(
            certfile=None, keyfile=None, purpose=ssl.Purpose.SERVER_AUTH,
            cafile=server_cert)

        # login dialog
        dlg = LoginDialog(self.master);  self.master.wait_window(dlg)
        if dlg.result is None: self.master.quit();  return
        self.username, self.password = dlg.result

        # first connection
        try:
            self._open_socket();  self._authenticate()
        except Exception as e:
            logger.error("connect/auth: %s", e)
    ...
    def _authenticate(self):
        ...
        if self._recv_prefixed() != b"USBREQ":
        ...
        while True:
            ...
            if reply.startswith("USBFAIL"):
                ...
                messagebox.showerror("USB Key", f"Wrong key – {left} attempt(s) left.")
                continue
            if reply.startswith("LOCKED"):
                mins = max(1, int(reply.split()[1]) // 60)
                messagebox.showerror("USB Key", f"Locked for {mins} minute(s).")
                raise SystemExit
    ...
    def _pick_usb_token() -> Optional[Tuple[str, str]]:
        ...
        for _ in range(4):
            drives = scan();  
            if drives: break;  time.sleep(0.5)
        else: return None

        if len(drives) == 1:
            _, s, d = drives[0];  return s, d

        win = ctk.CTkToplevel();  win.title("Select USB key");  win.grab_set()
        win.geometry("340x170");  choice: list[Tuple[str, str]] = []

        def pick(s, d): choice.append((s, d));  win.grab_release();  win.destroy()
        ...
        win.wait_window();  return choice[0] if choice else None

    # ── heartbeat ────────────────────────────────────────────────────
    def _heartbeat(self):
        while self.running:
            try: self._send_prefixed(b"PING")
    ...
    def _recv_prefixed(self) -> bytes:
        hdr = self.sock.recv(4);  length = int.from_bytes(hdr, "big") if hdr else 0
        ...
        while self.running:
            try:
                ...
                if text.startswith("USERS "):
                    self._update_user_list(text.split(" ", 1)[1].split(","))
                    continue
                self._display(text)

            except Exception as e:
                logger.warning("Connection lost: %s", e);  self.running = False
                if not messagebox.askyesno("Disconnected", "Lost connection.\nReconnect?"):
                    self.master.quit();  return
                if self._reconnect():  continue
                messagebox.showerror("Reconnect failed", "Unable to reconnect.");  self.master.quit();  return
    ...
    def _reconnect(self) -> bool:
        for attempt in range(1, self.RECONNECT_MAX_TRIES + 1):
            ...
            time.sleep(wait)
            try: self._open_socket();  self._authenticate();  return True
            except Exception as e: logger.warning("reconnect failed: %s", e)
        return False
    ...
    def _build_gui(self):
        ...
        self.master.grid_columnconfigure(1, weight=1);  self.master.grid_rowconfigure(1, weight=1)

        # sidebar
        sidebar = ctk.CTkFrame(self.master, width=250, fg_color="#212121")
        sidebar.grid(row=0, column=0, rowspan=3, sticky="ns", padx=(10, 0), pady=10)
        ...
        self.textbox = ctk.CTkTextbox(frame, fg_color="#1a1a1a",
                                      text_color="#00FF00",
                                      font=("Courier New", 18), state="disabled")
        ...
        self.entry = ctk.CTkEntry(input_fr, fg_color="#262626",
                                  text_color="#00FF00", font=("Helvetica", 18))
    ...
    def _update_user_list(self, users):
        for w in self.user_list.winfo_children(): w.pack_forget()
        self.user_labels = {}
        for u in ["Everyone"] + users:
        ...
        if self.recipient not in users + ["Everyone"]: self._set_recipient("Everyone")
    ...
    def _send(self, _=None):
        ...
        try:
            if self.recipient != "Everyone":
                self._send_prefixed(f"TO {self.recipient} {msg}".encode())
                shown = f"You ➜ {self.recipient}: {msg}"
            else:
                self._send_prefixed(f"{self.username}:{msg}".encode())
                shown = f"You: {msg}"
            self._display(shown);  self.entry.delete(0, "end")
        except Exception as e:
            logger.error("send failed: %s", e)
            messagebox.showerror("Send error", str(e));  self.master.quit()
