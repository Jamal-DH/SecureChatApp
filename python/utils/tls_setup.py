...
def ensure_cert_in_cert_dir(cert_filename="server_cert.pem", key_filename="server_key.pem"):
    ...
    if not os.path.exists(cert_path) or not os.path.exists(key_path):
        print("⚠️  Certificates not found. Generating new ones...")
...
def configure_tls_context(certfile, keyfile, purpose, cafile=None):
    ...
    try:
        if purpose == ssl.Purpose.SERVER_AUTH:
            # Configure a client-side TLS context
            context = ssl.create_default_context(ssl.Purpose.SERVER_AUTH)
            if cafile:
                context.load_verify_locations(cafile=cafile)
                context.verify_mode = ssl.CERT_REQUIRED  # Enforce certificate verification
            else:
                context.check_hostname = False  # Disable hostname verification (not recommended for production)
                context.verify_mode = ssl.CERT_NONE  # Allow connections without verifying server certificates
        else:
            # Configure a server-side TLS context
            context = ssl.SSLContext(ssl.PROTOCOL_TLS_SERVER)
            context.load_cert_chain(certfile, keyfile)
            context.verify_mode = ssl.CERT_OPTIONAL  # Allow but do not require client certificate authentication
